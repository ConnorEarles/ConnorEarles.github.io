...
function updateLoansArray() {
  ...
  //checking to see if the input is valid, if it is not then change the box to red
  let valid = true;
  ...
  //checking to see if the input is valid, if it is not then change the box to red
  if(valid) {
    //if the info is valid, then populate the array with the correct numbers
...
let updateForm = () => {
  ...
  for(i = 1; i < 6; i++) {
    //updating values in the form via a for loop
...
//making sure that the money has a dollar sign next to it
let toMoney = (value) => {  return `\$${toComma(value.toFixed(2))}`;} 

//save the current form
function SAVE(){
  //saving the information in the loans array
localStorage.setItem(`Current`, JSON.stringify(loans));
}

function LOAD(){
  //loading the current information in the loans array
